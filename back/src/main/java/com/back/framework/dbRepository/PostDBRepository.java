...
public interface PostDBRepository extends JpaRepository<PostEntity, Long> {
        ...
        void updateSell(Long id, String description, double price, String title, int type, Long originId, String image1,
                        String image2, String image3);

        @Query(value = "SELECT COUNT(c.*) FROM comments c WHERE post_id=?1", nativeQuery = true)
