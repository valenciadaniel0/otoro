...
public interface CityDBRepository extends JpaRepository<CityEntity, Long> {
    // @Query(value = "SELECT c.id,c.code, concat(c.name,'-',d.name),
    // c.department_id FROM cities c, departments d WHERE c.department_id= d.id AND
    // concat(c.name,'-',d.name) LIKE '%?1%'", nativeQuery = true)
