...
public class UserController {
    ...
    public ResponseEntity createAuthenticationToken(@RequestBody UserCommand userCommand) throws Exception {
        ...
        final UserDetails userDetails = userRepositoryImplementation.loadUserByUsername(userCommand.getUsername());
        User user = this.getUserByEmailHandler.run(userCommand.getUsername());
        final String token = jwtTokenUtil.generateToken(userDetails);

        return ResponseEntity
                .ok(new JwtResponse(token, user.getName(), user.getEmail(), user.getActive(), user.getDeviceToken()));
    }
